...
A diferencia de enforos y liberamerkato, **la API de este proyecto no arma la conexión con las variables `DB_*` del entorno**. `config/common.php` fija `host: localhost`, `dbname: cooperativismoabierto` y `user`/`password` en `null`, y eso se pisa desde `config/config.php`, que está en `.gitignore`.
...
`/var/www/cooperativismoabierto/api/config/config.php` con los datos de la base (ver arriba).
